    ...
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.nav}>
      {items.map((item) => {
        ...
        return <Pressable key={item.key} onPress={() => onChange(item.key)} style={[styles.navItem, active && styles.navItemActive]}>
          <View><Feather name={item.icon} size={16} color={active ? c.primary : c.mutedForeground} />{item.key === 'chat' && unread > 0 && <View style={styles.badge}><Text style={styles.badgeText}>{unread > 9 ? '9+' : unread}</Text></View>}</View>
...
export default function CloudPrivateWorldApp() {
  const { currentUser, isLoading, logout, messages, settings } = useCloudApp();
  ...
  return <View style={[styles.root, { paddingTop: insets.top }]}><Header onSettings={() => setSettingsOpen(true)} onLogout={() => void logout()} /><Navigation section={section} onChange={setSection} unread={unread} /><View style={styles.screen}>{screen}</View>{composer && <Composer kind={composer} onClose={() => setComposer(null)} />}{edit && <EditSheet kind={edit.kind} item={edit.item} onClose={() => setEdit(null)} />}<SettingsSheet visible={settingsOpen} onClose={() => setSettingsOpen(false)} /><FullscreenPhoto photo={photo} onClose={() => setPhoto(null)} />{settings?.finalMessage ? <Text style={styles.finalMessage}>{settings.finalMessage}</Text> : null}</View>;
}
...
const styles = StyleSheet.create({
  ...
  nav: { gap: 6, paddingBottom: 10, paddingHorizontal: 18 }, navItem: { alignItems: 'center', borderRadius: 16, flexDirection: 'row', gap: 6, minHeight: 38, paddingHorizontal: 12 }, navItemActive: { backgroundColor: c.secondary }, navText: { color: c.mutedForeground, fontSize: 12, fontWeight: '700' }, navTextActive: { color: c.primary }, badge: { alignItems: 'center', backgroundColor: c.primary, borderColor: c.background, borderRadius: 8, borderWidth: 2, height: 17, justifyContent: 'center', minWidth: 17, position: 'absolute', right: -10, top: -9 }, badgeText: { color: '#fff', fontSize: 8, fontWeight: '800' },
